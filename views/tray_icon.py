...
from views.about_view import AboutCometDialog
from views.update import CheckUpdateDialog, NoUpdateDialog
...
from utils.tools import dc, log, config, env, show_toast
...
class TrayIcon(QSystemTrayIcon):
    ...
    def setTrayIcon(self):
        ...
        if config.getVal("Debug", "window") == "1":
            attr_title = "-debug模式"
        self.setToolTip(env.TRAY_ICON_TOOLTIP + attr_title)
    ...
    def update_device_info(self, device_info):
        ...
        if config.getVal("Debug", "window") == "1":
            attr_title = "-debug模式"
        self.setToolTip(env.TRAY_ICON_TOOLTIP + attr_title + tip_text)
        ...
        if response["status_code"] == 200:
            ...
            if latest_version > current_version:
                ...
                show_toast(
                    self.parent(),
                    "提示",
                    f"已经是最新版本了,当前版本: {env.APP_VERSION}",
                    "info",
                )
    ...
    def cleanup(self):
        ...
        # 停止定时器
        if hasattr(self, '_hover_timer') and self._hover_timer and self._hover_timer.isActive():
            self._hover_timer.stop()
            self._hover_timer.deleteLater()

        # 清理对话框
        if hasattr(self, 'about') and self.about:
            self.about.close()
            self.about.deleteLater()
            self.about = None

        if hasattr(self, 'set_app') and self.set_app:
            self.set_app.close()
            self.set_app.deleteLater()
            self.set_app = None

        if hasattr(self, 'checkUpdateDialog') and self.checkUpdateDialog:
            self.checkUpdateDialog.close()
            self.checkUpdateDialog.deleteLater()
            self.checkUpdateDialog = None

        if hasattr(self, 'noUpdateDialog') and self.noUpdateDialog:
            self.noUpdateDialog.close()
            self.noUpdateDialog.deleteLater()
            self.noUpdateDialog = None
        ...
        # 清理皮肤菜单
        if hasattr(self, 'taskSkin') and self.taskSkin:
            self.taskSkin.clear()
            self.taskSkin.deleteLater()
            self.taskSkin = None

        # 清理设备动作
        if hasattr(self, 'device_actions'):
            self.device_actions.clear()

        if hasattr(self, 'skin_actions'):
            self.skin_actions.clear()
...
if __name__ == "__main__":
    ...
    tray = TrayIcon(main_window, SkinManager("skin/ring/", "Ring"))
    tray.show()
